...
must_haves:
  truths:
    ...
    - "Business name extracted correctly (empty/malformed flagged)"
    - "Website URL captured (with sourceUrl fallback)"
  artifacts:
...
- **CRITICAL: Empty or malformed name: -50 points, add flag "empty_or_malformed_name"**
...
- `isEmptyOrMalformedName(name: string | null | undefined): boolean` - NEW
...
4. Implement `isEmptyOrMalformedName`:
```typescript
function isEmptyOrMalformedName(name: string | null | undefined): boolean {
  if (!name) return true;
...
5. Return `isLikelyBusiness: true` if final score >= 60
...
Verify name validation: `grep -n "isEmptyOrMalformedName\|empty_or_malformed_name" lib/scraper/validation.ts`
  </verify>
  <done>
...
- At least 5 negative signal checks implemented (including empty/malformed name)
- isEmptyOrMalformedName helper function implemented
...
- **Website URL falls back to sourceUrl when null**
  </action>
  <verify>
...
Verify website fallback: `grep -n "sourceUrl" app/api/admin/scraped-businesses/[id]/route.ts`
  </verify>
  <done>
...
- PUT handler uses sourceUrl as website fallback when website is null
...
3. Name validation implemented:
   ```bash
   grep -n "isEmptyOrMalformedName\|empty_or_malformed_name" lib/scraper/validation.ts
   ```

4. Website fallback implemented:
   ```bash
   grep -n "sourceUrl" app/api/admin/scraped-businesses/[id]/route.ts
   ```

5. Approval handler includes photo/tag transfer:
   ```bash
...
- [ ] Empty/malformed business names receive -50 point penalty and flag
...
- [ ] Approval handler uses sourceUrl as website fallback
