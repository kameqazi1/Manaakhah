...
must_haves:
  ...
  key_links:
    ...
    - from: "lib/scraper/scraper.ts"
      to: "lib/scraper/utils.ts"
      via: "checkForDuplicateInDatabase call before db.scrapedBusiness.create"
      pattern: "checkForDuplicateInDatabase"
---
...
<task type="auto">
  <name>Task 3: Wire deduplication into scraper save flow</name>
  <files>lib/scraper/scraper.ts</files>
  <action>
Wire the `checkForDuplicateInDatabase` function into the scraper's save flow so it actually checks for duplicates before creating ScrapedBusiness records.

1. Add import at top of scraper.ts:
```typescript
import { checkForDuplicateInDatabase } from "./utils";
```

2. Find the function that saves scraped businesses to the database. Look for `db.scrapedBusiness.create` or similar patterns.

3. Before each `db.scrapedBusiness.create` call, add the deduplication check:

```typescript
// Before saving a transformed business:
const duplicateCheck = await checkForDuplicateInDatabase(transformedBusiness);

if (duplicateCheck.isDuplicate) {
  console.log(
    `Skipping duplicate: ${transformedBusiness.name} ` +
    `(matched ${duplicateCheck.matchType} table via ${duplicateCheck.matchField}, ` +
    `id: ${duplicateCheck.existingId})`
  );
  // Skip this business or update existing record
  stats.duplicatesSkipped = (stats.duplicatesSkipped || 0) + 1;
  continue; // or return early depending on code structure
}

// Proceed with create if not duplicate
await db.scrapedBusiness.create({
  data: transformedBusiness,
});
```

4. Update the scraper stats/return value to include duplicatesSkipped count.

5. The check should happen in the main scrape loop, typically in:
   - `runScraper` function
   - Or wherever businesses are batched and saved

Look for patterns like:
- `for (const business of businesses)`
- `businesses.map(async (b) => ...)`
- Any loop that processes scraped results before saving
  </action>
  <verify>
Run TypeScript check: `npx tsc --noEmit lib/scraper/scraper.ts`
Verify import: `grep -n "checkForDuplicateInDatabase" lib/scraper/scraper.ts`
Verify call site: `grep -B2 -A2 "checkForDuplicateInDatabase" lib/scraper/scraper.ts`
  </verify>
  <done>
- checkForDuplicateInDatabase imported in scraper.ts
- Function called before db.scrapedBusiness.create
- Duplicate businesses are skipped with console log
- Stats include duplicatesSkipped count
  </done>
</task>
...
5. Deduplication wired into scraper:
   ```bash
   grep -n "checkForDuplicateInDatabase" lib/scraper/scraper.ts
   ```
</verification>
...
- [ ] checkForDuplicateInDatabase is called before saving scraped businesses
- [ ] Duplicate businesses are skipped (not saved)
